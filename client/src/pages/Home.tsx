...
type Partner =
  ...
  | { kind: "icon";   name: string; icon: "linkedin" }
  | { kind: "flag-qc"; name: string };

const PARTNERS: Partner[] = [
  { kind: "logo",    name: "Ordre des CRHA",         logo: "https://logo.clearbit.com/ordrecrha.org" },
  { kind: "icon",    name: "LinkedIn",                icon: "linkedin" },
  { kind: "logo",    name: "Service Canada",          logo: "https://upload.wikimedia.org/wikipedia/commons/8/83/Canada_wordmark.svg" },
  { kind: "flag-qc", name: "Gouvernement du Québec" },
  { kind: "flag-qc", name: "Emploi-Québec" },
  { kind: "flag-qc", name: "CPMT Québec" },
  { kind: "logo",    name: "CRIA",                   logo: "https://logo.clearbit.com/ordrecrha.org" },
];

function PartnerLogo({ p }: { p: Partner }) {
  if (p.kind === "icon") {
    return (
      <span className="flex items-center gap-1.5 text-foreground/30 select-none whitespace-nowrap">
        <SiLinkedin size={18} />
  ...
  return (
    <span className="inline-flex items-center select-none">
      ...
      />
      <span
        className="font-semibold text-[12px] whitespace-nowrap text-foreground/30"
        style={{ display: "none" }}
      >
        {p.name}
      </span>
